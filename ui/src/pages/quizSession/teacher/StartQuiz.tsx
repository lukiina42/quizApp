...
const StartQuiz = (props) => {
  ...
  const [currentLayout, setCurrentLayout] = useState<string>(LayoutType.UsersJoining)
  ...
  const [currentQuestionEvaluation, setCurrentQuestionEvaluation] = useState<QuestionEvaluationType | null>(null)
  //Student scores, comes from server at the end of the quiz, only top 3 are displayed
  const [studentScores, setStudentScores] = useState<StudentScoresType | null>(null)
  //Holds info about how many students answered to current tested question out of how many
  const [studentsAnsweredInfo, setStudentsAnsweredInfo] = useState<StudentAnsweredType | null>(null)
  ...
  const currentQuestion = useMemo(() => getTheQuestionByKey(quiz, currentQuizKey), [currentQuizKey, quiz])
  ...
  const sendRequestToEndTheQuiz = (reasonParameter: string, lastSessionId?:number): void => {
  ...
  useEffect(() => {
    if(lastQuestion && (currentLayout === LayoutType.ShowEvaluation)){
      ...
      stompClient.send("/ws/getStudentResults", {}, JSON.stringify(getStudentResultsRequest))
    }
  ...
  const handleNextQuestionButton = () => {
    ...
    stompClient.send("/ws/nextQuestion", {}, JSON.stringify(nextMessageRequest))
  ...
  const handleEndQuestionButton = () => {
    ...
    stompClient.send("/ws/getEvaluation", {}, JSON.stringify(getEvaluationRequest))
  }
  ...
  const handleShowStudentResults = () => {
    ...
    stompClient.send("/ws/sendResults", {}, JSON.stringify(getEvaluationRequest))
  ...
  return (
    <>
      <Prompt
        when={!(lastQuestion && (LayoutType.ShowEvaluation === currentLayout || LayoutType.StudentScores === currentLayout))}
            ...
            <Grid container justifyContent={"center"} sx={{width: '100%'}}>
              {
                users.map(user =>
                  <Grid item key={user}>
                    <ListItemButton 
                      onMouseEnter={event => event.currentTarget.style.cursor = "default"}
                    >
                      <ListItemIcon>
                        <PersonIcon />
                      </ListItemIcon>
        ...
        >
          <Grid item xs={11} sx={{width: "100%", minHeight: "630px"}}>
            <Grid container direction={"column"} alignItems="center" justifyContent={"center"} sx={{width: "100%", height: "100%"}}>
              ...
              <Grid item xs={1}>
                <Typography fontWeight={"bold"} fontSize={"20px"}>Amount of students who answered / total students :&nbsp;
                  {studentsAnsweredInfo ? studentsAnsweredInfo.amountOfAnswers : 0} /&nbsp;
                  {studentsAnsweredInfo ? studentsAnsweredInfo.amountOfStudents : users.length}
                </Typography>
              </Grid>
            </Grid>
          </Grid>
          <Grid item xs sx={{minHeight: "50px", width: "100%", textAlign: "center"}}>
            <Button
              variant='contained'
              color='primary'
