...
const QuestionCreator = (props) => {
  const {setCurrentQuestions, questionParams, setQuestionParams,
    answersCorrect, setAnswersCorrect, answersValues, setAnswersValues, 
    handleAnswerValueChange, validate, disabled, questionName, codeTextProp, languageProp} = props
  ...
  const [language, setLanguage] = useState(disabled ? languageProp : languageTypes.C)
      ...
      body: JSON.stringify(
        {
          ...
          questions: finalQuestions}), // body data type must match "Content-Type" header
        }
      )
    .then(response => {
  ...
  return (
    <>
      ...
      >
        ...
        <Grid item xs={1} sx={{display: "flex", minHeight:'58px', justifyContent: "center", alignItems: "center", width:'80%'}}>
          <TextField
            {...nameDynamicValue}
            id="name"
            size="small"
        ...
        <Grid item xs width={'80%'} sx={disabled ? {maxHeight: "340px"} : {minHeight: language === languageTypes.PLAINTEXT ? '210px' : '326px' }}>
          <Grid
           container 
           direction={"row"}
           spacing={0}
          ...
          >
            <Grid item xs={disabled ? 12 : 10}>
              ...
              <TextField
                ...
                fullWidth
                value={codeText}
                onChange={!disabled ? handleQuestionPlainTextChange : () => {}}
                className={classes.textField}
                disabled={disabled}
              />
              :
        ...
        <Grid item xs={4} width={'80%'} sx={{minHeight: '225px', display:'flex', alignItems:'center'}}>
          <Answers
        ...
        >
          <Grid
            ...
            alignItems={"center"}
            sx = {{ height:'100%', witdth:'100%', minHeight:'70px', borderTop: 1}}
          >
            <Button 
              color="primary" 
              variant="contained" 
