...
import { useUser, UserStatus, useUserUpdate, initialLoggedUserState } from "../context/UserContext"
...
export default function Header(props) {
  ...
  return (
    <>
      {(location.pathname === '/login' || location.pathname === '/registration' || location.pathname === '/startQuiz') ? 
      <>
      </>
      :
      <>
          ...
          >
            ...
            */}
            <Grid item  sx={{paddingLeft:2}}>
              {(location.pathname === '/' && currentUser.status === UserStatus.Logged) && 
